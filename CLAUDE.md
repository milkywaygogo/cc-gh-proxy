...
- **Opus pinning**: every `claude-opus-*` request resolves to a single model (`--opus-model`, default `claude-opus-5`) regardless of the version Claude Code asks for, so a hardcoded dated model ID can't silently select an older Opus. Sonnet and Haiku keep normal version mapping.
