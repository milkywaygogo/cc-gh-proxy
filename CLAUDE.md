...
- **Model name mapping**: Claude Code sends dashes (`claude-sonnet-4-6`), Copilot expects dots (`claude-sonnet-4.6`). Bracket suffixes (`[1m]`), date suffixes, and base family names also handled.
- **Opus pinning**: every `claude-opus-*` request resolves to a single model (`--opus-model`) regardless of the version Claude Code asks for, so a hardcoded dated model ID can't silently select an older Opus. Sonnet and Haiku keep normal version mapping.
- **Startup model discovery**: on boot (when talking to Copilot), `GET /models` and pick the newest `claude-opus-*` / `claude-sonnet-*` / `claude-haiku-*` as defaults for the opus pin, `--no-opus-target`, and `claude-{sonnet|haiku}-{major}` family map. Explicit `--opus-model` / `--no-opus-target` (or their env vars) win; on fetch failure the hardcoded fallbacks stay.
